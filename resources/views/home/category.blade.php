        ...
        <div class="col-sm-12">
            <nav aria-label="breadcrumb" style="text-align:left; background:black; border-radius: 10px 10px 0 0;padding-left:10px">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a style="text-decoration: none; {{$Categories2 == null ? 'color: #E06F18;' : ''}}" href="{{route('home').'/'.$Products->category['tag']}}">{{$Products->category['name']}}</a></li>
                    @if ($Categories2 != null)
                    <li class="breadcrumb-item"><a style="text-decoration: none; color: #E06F18;" href="{{route('home').'/'.$Products->category['tag'].'/'.$Products->category2['tag']}}">{{$Products->category2['name']}}</a></li>
                    @endif
                </ol>
            </nav>	
            <div>
            ...
            <ul class="pagination justify-content-center" id="pagination">
                @if ($Page > 1)
                    <li class="page-item"><a class="page-link" style="color: black;" href="{{route('home').'/'.$Products->category['tag']}}{{$Categories2 != null ? '/'.$Products->category2['tag'] :''}}?page=1&sort={{$Sort}}">Trang đầu</a></li>
                @endif
                @for ($i = 1; $i <= $Count_page; $i++)
                    <li class="page-item {{$Page==$i ? 'disabled' : ''}}"><a class="page-link" style="{{$Page==$i ? 'color: #E06F18' : 'color: black'}}" href="{{route('home').'/'.$Products->category['tag']}}{{$Categories2 != null ? '/'.$Products->category2['tag'] :''}}?page={{$i}}&sort={{$Sort}}">{{$i}}</a></li>
                @endfor
                @if ($Page < $Count_page)
                    <li class="page-item"><a class="page-link" style="color: black;" href="{{route('home').'/'.$Products->category['tag']}}{{$Categories2 != null ? '/'.$Products->category2['tag'] :''}}?page={{$Count_page}}&sort={{$Sort}}">Trang cuối</a></li>
                @endif 
            </ul>
            @endif 
        </div> 
...
<script>
    $(document).ready(function(){
        $('#filter').change(function(){       
            ...
            window.location.assign(url+'?page=1&sort='+$(this).val());
        })
    })
</script>
@endsection